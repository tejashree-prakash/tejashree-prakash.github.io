...
### Chart A: Total Precipitation Bar Graph
```{r}
#Sum precipitation by station
precip_sum <- noaa %>%
  filter(!is.na(prcp)) %>%
  group_by(id) %>%
...
#Produce bar plot
plot_ly(precip_sum, x = ~id, y = ~total_prcp, type = "bar",
        text = ~total_prcp, textposition = "auto") %>%
  layout(title = "Total Precipitation by Weather Station",
         xaxis = list(title = "Station ID"),
         yaxis = list(title = "Total Precipitation (tenths of mm)"))
...
### Chart B: Scatterplot showing Snow Fall vs Snow Depth 
```{r}
...
### Chart C: Time Series of Temperature Values 
```{r}
...
plot_ly(subset_data, x = ~date, y = ~tmax,
  ...
  layout(title = "Temperature Observations at Selected Weather Stations",
         xaxis = list(title = "Date (year)"),
         yaxis = list(title = "Temperature (tenths of °C)"),
         legend = list(title = list(text = "Station ID")))
```
